...
function volcarDatostxt($tvalores){
    $fich = @fopen(FILEUSER, 'w') or die("ERROR al abrir fichero de usuarios"); // abrimos el fichero para escritura
    for ($i=0; $i < count($tvalores); $i++) { 
        $parte=implode('|', $tvalores[$i])."\n";
        fwrite($fich, $parte);
    }
    fclose($fich);
}
...
function cargarDatoscsv (){
    ...
    $file = @fopen(FILEUSER, 'r') or die("ERROR al abrir fichero de usuarios"); 
...
function cargarDatosjson (){
    ...
    $mitabla= json_decode($userJSON, true);
